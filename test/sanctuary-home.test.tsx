...

{
  ...
  ok(
    page.includes('if (process.env.NEXT_PUBLIC_SANCTUARY !== "1") notFound();'),
    "Home is behind the sanctuary flag, the same hard 404 as the chamber and the pothi (D1.1); comparing against the literal \"1\" is what stops an empty or accidental value opening it",
  );
  ok(
    !page.includes("NODE_ENV") && !page.includes("SNC_MEASURE"),
    "and the flag is its only key: the development gate and the SNC_MEASURE lift are gone — scripts/capture/ builds with the flag like any deploy",
  );
